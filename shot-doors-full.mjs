...
await page.evaluate(() => {
  ...
  if (sec) { const sc = document.querySelector('[data-inn-scroll]'); const r = sec.getBoundingClientRect(); sc.scrollTop += r.top; }
});
await page.waitForTimeout(800);
await page.screenshot({ path: '/tmp/FINAL-doors.png', fullPage: false });
const info = await page.evaluate(() => {
  ...
  return rect ? { x: rect.x, y: rect.y, right: rect.right, bottom: rect.bottom, w: rect.width, h: rect.height, vw: window.innerWidth, vh: window.innerHeight } : null;
});
console.log('h2 rect:', JSON.stringify(info));
await browser.close();
