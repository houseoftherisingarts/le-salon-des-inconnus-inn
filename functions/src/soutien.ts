...
import { RUNTIME_WITH_SMTP, notifyAlex, line, envoyerAuMembre } from './courriel';
...
export const onMessageSoutien = functions
      ...
      try {
        ...
        if (prefs?.courrielReponseEquipe !== false) {
          ...
          if (authUser.emailVerified && authUser.email) {
            ...
            await envoyerAuMembre(
              authUser.email,
              langue === 'EN' ? 'The Salon team has replied' : 'L\'équipe du Salon vous a répondu',
              corps,
              LIEN_AIDE,
              bouton,
              pied,
...
export const onProblemeTechnique = functions
  .runWith(RUNTIME_WITH_SMTP)
  ...
  .onCreate(async (snap) => {
    ...
    await notifyAlex(
      `Problème technique signalé par ${nom}`,
      line('Nom', p.nom) +
      line('Courriel', p.courriel) +
      line('Page', `https://www.lesalondesinconnus.com${p.page ?? ''}`) +
      line('Agent', p.agent) +
      line('Écran', p.ecran) +
      (p.capture ? line('Capture', p.capture) : '') +
      `\n${texte.slice(0, 2000)}\n` +
...
export const onMessageEntreMembres = functions
  .runWith(RUNTIME_WITH_SMTP)
  ...
  .onCreate(async (snap, context) => {
    ...
    const membres: string[] = Array.isArray(conv.members) ? conv.members : [];

    let auteurNom = 'Un membre';
    try {
      auteurNom = (await getAuth().getUser(auteurUid)).displayName ?? 'Un membre';
    } catch { /* nom indisponible */ }

    for (const uid of membres) {
      if (uid === auteurUid) continue;
      try {
        const prefs = (await db.doc(`members/${uid}/prive/preferences`).get()).data();
        if (prefs?.courrielNouveauMessage !== true) continue;
        ...
        const dernier = (envois as Record<string, { toMillis?: () => number }>)[convId]?.toMillis?.() ?? 0;
        if (Date.now() - dernier < RATELIMIT_MS) continue;

        const authUser = await getAuth().getUser(uid);
        if (!authUser.emailVerified || !authUser.email) continue;
        ...
        await envoyerAuMembre(
          authUser.email,
          langue === 'EN' ? `${auteurNom} wrote to you at the Salon` : `${auteurNom} vous a écrit au Salon`,
          corps,
          LIEN_MESSAGES,
          bouton,
          pied,
        ...
        await db.doc(`members/${uid}/prive/envois`).set(
          { [convId]: admin.firestore.FieldValue.serverTimestamp() },
          { merge: true },
        );
      } catch (e) {
