...
export const onWwooferApplication = functions
  .runWith(RUNTIME_WITH_SMTP)
  .firestore.document('wwoofers/{uid}')
  .onCreate(async (snap) => {
    const w = snap.data() ?? {};
    const body =
      `Nouvelle candidature wwoofer.\n\n` +
...
export const onWwooferVisitRequest = functions
  .runWith(RUNTIME_WITH_SMTP)
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      const p = parent.data();
      if (p?.displayName) who = `${p.displayName}${p.email ? ` (${p.email})` : ''}`;
    } catch { /* fall back to uid */ }
    const body =
      `Nouvelle demande de dates wwoofer.\n\n` +
      line('Wwoofer', who) +
      line('Du', r.startDate) +
      line('Au', r.endDate) +
...
export const onShowOffer = functions
  .runWith(RUNTIME_WITH_SMTP)
  .firestore.document('events/{eventId}/showOffers/{offerId}')
  .onCreate(async (snap, context) => {
    const o = snap.data() ?? {};
    const body =
      `Nouvelle offre de spectacle (événement ${context.params.eventId}).\n\n` +
...
export const stripeCampingWebhook = onRequest(
  { secrets: [STRIPE_WEBHOOK_SECRET, 'ZOHO_USER', 'ZOHO_PASS'], cors: false },
  async (req, res) => {
    if (req.method !== 'POST') {
    ...
    const nouveau = await db.runTransaction(async (tx) => {
      ...
      tx.set(
        compteurRef,
        { vendus: Math.min(CAMPING_PLACES, vendus + 1), updatedAt: admin.firestore.FieldValue.serverTimestamp() },
        { merge: true },
      );
      return true;
    });
