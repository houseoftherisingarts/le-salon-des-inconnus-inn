...
const GlassFrame: React.FC<{ src: string; alt: string; eager?: boolean; className?: string }> = ({
  ...
  <div
    className={`rounded-[15px] border border-[#c5a059]/25 bg-black/40 backdrop-blur-md shadow-[0_0_60px_rgba(197,160,89,0.15)] overflow-hidden ${className}`}
  >
...
export const CoffreLanding: React.FC<Props> = ({ onNavigate, language }) => {
  ...
  const bands: Band[] = [
    ...
    },
    {
      ...
      img: 'paliers.webp',
      alt: [
        'Treasure milestones and the lifetime earnings line.',
        'Les paliers du trésor et la ligne des gains à vie.',
      ],
      ctaAfter: true,
    },
    {
  ...
  const faq: [string, string, string, string][] = [
    [
      'From what age?',
      'From 4 or 5 at level 1, where the child counts coins. The ten levels climb all the way to adult financial education.',
      'À partir de quel âge ?',
      "Dès 4 ou 5 ans au niveau 1, où l'enfant compte les sous. Les dix niveaux montent jusqu'à l'éducation financière adulte.",
    ],
    [
      'Do I need to know finance?',
      'No. Every level comes with guideposts for the parent, and the built-in guide explains each module, in French and in English.',
      "Faut-il s'y connaître en finances ?",
      "Non. Chaque niveau vient avec ses repères pour le parent, et le guide intégré explique chaque module, en français et en anglais.",
    ],
    [
      'Is it really free?',
      'Yes. The public beta is free and ad-free. Donations and sponsors keep the project alive.',
      "C'est vraiment gratuit ?",
      "Oui. La beta publique est gratuite et sans publicité. Les dons et les commanditaires gardent le projet vivant.",
    ],
    [
      'Where does the data go?',
      'Nowhere. Everything stays on your device, and you can create timestamped backups whenever you want.',
      'Où vont les données ?',
      "Nulle part. Tout reste sur votre appareil, et vous pouvez créer des sauvegardes horodatées quand vous voulez.",
    ],
    [
      'On which devices?',
      'In the browser, on macOS and Windows, and on phones or tablets by adding it to the home screen.',
      'Sur quels appareils ?',
      "Dans le navigateur, sur macOS et Windows, et sur téléphone ou tablette en l'ajoutant à l'écran d'accueil.",
    ],
  ];

  return (
    <div
      className="fixed inset-0 z-50 w-full h-full overflow-y-auto scroll-smooth text-neutral-200"
      style={{ background: '#0a0808' }}
    >
      <div className="fixed inset-0 overflow-hidden pointer-events-none" style={{ zIndex: -1 }} aria-hidden>
        <div
          className="absolute inset-0"
          style={{ background: 'radial-gradient(120% 90% at 50% 0%, #14100c 0%, #0a0808 55%, #060505 100%)' }}
        />
        <div
          className="absolute inset-0"
          style={{ background: 'radial-gradient(48% 38% at 50% 16%, rgba(201,168,90,0.10), transparent 72%)' }}
        />
      ...
      <main>
        ...
        <section className="max-w-7xl mx-auto px-6 pt-36 md:pt-40 pb-24 grid md:grid-cols-2 gap-14 items-center">
          <div>
            <motion.p
              ...
              className="text-[11px] tracking-[0.3em] uppercase text-neutral-500 mb-5"
            >
            ...
            <motion.h1
              ...
              className="font-cinzel text-4xl md:text-6xl text-[#e8d5a3] leading-tight mb-6"
            >
            ...
            >
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <a href="#telecharger" className={PILL_SECONDARY}>
              ...
              <p className="text-xs text-neutral-500 mt-5">
                {t(
                  ...
                  'Gratuit · Sans compte · Tout reste sur votre appareil',
                )}
              </p>
            </motion.div>
          </div>
        ...
        <motion.section {...reveal()} className="border-t border-white/5 py-20 md:py-28">
          <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] gap-10">
            <h2 className="font-cinzel text-3xl text-[#e8d5a3]">
              {t('School will not teach it.', "L'école ne l'enseignera pas.")}
            </h2>
          ...
          <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {[
                {
                  n: '1',
                  title: t('Open the Coffre', 'Ouvrez le Coffre'),
                  body: t(
                ...
                {
                  n: '2',
                  title: t('Deposit the allowance', "Déposez l'allocation"),
                  body: t(
                ...
                {
                  n: '3',
                  title: t('Pay the interest', "Versez l'intérêt"),
                  body: t(
                ...
                <motion.div
                  key={card.n}
                  {...reveal(i * 0.1)}
                  className="rounded-[15px] border border-[#c5a059]/25 bg-black/40 backdrop-blur-md p-8"
                >
        ...
        {bands.map((b) => (
          <motion.section key={b.key} {...reveal()} className="border-t border-white/5 py-20 md:py-28">
            <div className="max-w-7xl mx-auto px-6">
              <div
                className={`grid md:grid-cols-[1.1fr_0.9fr] gap-12 items-center ${b.reverse ? 'md:[&>*:first-child]:order-2' : ''}`}
              >
                <div className="flex items-center">
                  <GlassFrame
                    src={`${M}${b.img}`}
                    alt={t(b.alt[0], b.alt[1])}
                    className={`w-full ${b.imgClass || ''}`}
                  />
                </div>
                <div>
        ...
        <motion.section {...reveal()} className="border-t border-white/5 py-20 md:py-28">
          <div className="max-w-7xl mx-auto px-6">
            <p className="text-[11px] tracking-[0.3em] uppercase text-[#c5a059] mb-4 text-center">
              {t('Ten levels', 'Dix niveaux')}
            </p>
            <h3 className="font-cinzel text-3xl text-[#e8d5a3] mb-5 text-center">
              {t('A bank that grows with your child.', 'Une banque qui grandit avec votre enfant.')}
            </h3>
              ...
              {Array.from({ length: 10 }, (_, i) => i + 1).map((n) => {
                ...
                return (
                  <div key={n} className="flex flex-col items-center gap-2 w-20">
                    <div
                      className="w-12 h-12 rounded-full flex items-center justify-center font-cinzel text-[#171308] font-bold"
                      style={{ background: 'linear-gradient(135deg, #c5a059, #e8d5a3, #c5a059)' }}
                    >
                      {n}
                    </div>
        ...
        <motion.section {...reveal()} className="border-t border-white/5 py-20 md:py-28">
          <div className="max-w-7xl mx-auto px-6">
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div>
                <h3 className="font-cinzel text-3xl text-[#e8d5a3] mb-5">
                  {t('Built by a family, not by a bank.', 'Bâti par une famille, pas par une banque.')}
                </h3>
                <p className="text-neutral-300 leading-relaxed">
                  {t(
              ...
              <GlassFrame
                src={`${M}wizard.webp`}
                alt={t('The welcome assistant guiding a new family through the Coffre.', "L'assistant d'accueil qui guide une nouvelle famille dans le Coffre.")}
              />
            </div>
            <div className="mt-10 pt-8 border-t border-[#c5a059]/15 flex flex-wrap items-center justify-center gap-3 text-xs text-neutral-500">
              ...
              <a href={SPONSOR} className="underline decoration-dotted underline-offset-4 hover:text-[#e8d5a3] transition-colors">
        ...
        <motion.section {...reveal()} className="border-t border-white/5 py-20 md:py-28">
          <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-14">
            <div>
              <h3 className="font-cinzel text-3xl text-[#e8d5a3] mb-5">
                {t('Nothing to lose, everything to learn.', 'Rien à perdre, tout à apprendre.')}
              </h3>
              <p className="text-neutral-300 leading-relaxed">
                {t(
        ...
        <motion.section id="telecharger" {...reveal()} className="border-t border-white/5 py-24 md:py-32">
          <div className="max-w-4xl mx-auto px-6 rounded-[15px] border border-[#c5a059]/25 bg-black/40 backdrop-blur-md p-10 md:p-16 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              ...
              <a href={DL_MAC} className={PILL_SECONDARY}>
                {t('Download for macOS', 'Télécharger pour macOS')}
              </a>
              <a href={DL_WIN} className={PILL_SECONDARY}>
                {t('Download for Windows', 'Télécharger pour Windows')}
              </a>
              <a href={DL_MOBILE} target="_blank" rel="noopener noreferrer" className={PILL_SECONDARY}>
