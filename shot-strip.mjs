...
await open('02');
await page.screenshot({ path: '/tmp/STRIP-prog.png', fullPage: false });

await page.evaluate(() => {
  const fs = document.querySelector('.chapter-fullscreen');
  fs?.querySelector('button[aria-label*="Close"], button[aria-label*="Fermer"]')?.click();
});
await page.waitForTimeout(500);

await open('05');
await page.screenshot({ path: '/tmp/STRIP-prac-top.png', fullPage: false });
await page.evaluate(() => { const el = document.querySelector('.chapter-fullscreen'); if (el) el.scrollTop = 800; });
await page.waitForTimeout(500);
await page.screenshot({ path: '/tmp/STRIP-prac-stubs.png', fullPage: false });
await browser.close();
console.log('done');
